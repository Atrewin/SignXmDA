...
import socket
from signjoey.dataset import SignTranslationDataset, Gloss2TextDataset
...
def load_data(data_cfg: dict) -> (Dataset, Dataset, Dataset, Vocabulary, Vocabulary):
    ...
    if data_cfg.get("gloss2text_data", False) != False:
        train_g2t_path = data_cfg.get("gloss2text_data", False)

        train_gloss2text_data = Gloss2TextDataset(
            path=train_g2t_path,
            fields=(gls_field, txt_field),
            filter_pred=lambda x: len(vars(x)["txt"]) <= max_sent_length,
        )
    else:
        train_gloss2text_data = None
    ...
    gls_vocab = build_vocab(
        ...
        dataset_list=[train_data, train_gloss2text_data],
    ...
    return train_data, dev_data, test_data, gls_vocab, txt_vocab, train_gloss2text_data

def load_data_raw(data_cfg: dict) -> (Dataset, Dataset, Dataset, Vocabulary, Vocabulary):
    """
    Load train, dev and optionally test data as specified in configuration.
    Vocabularies are created from the training set with a limit of `voc_limit`
    tokens and a minimum token frequency of `voc_min_freq`
...
def make_data_iter(
    ...
    if train:
        ...
        data_iter = data.BucketIterator(
            ...
            sort_key=lambda x: len(getattr(x, sort_key)),
            shuffle=shuffle,
        )
    else:
